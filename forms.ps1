...
function New-OpenFileDialog {
    param (
        [Parameter(mandatory=$false)][System.String]$InitialDirectory,
        [Parameter(mandatory=$false)][switch]$Multiselect,
        [Parameter(mandatory=$false)][System.String]$Filter
    )
    [System.Windows.Forms.OpenFileDialog]$oNewOpenFileDialog = New-Object System.Windows.Forms.OpenFileDialog
    if ($InitialDirectory){
        $oNewOpenFileDialog.InitialDirectory = $InitialDirectory
    }
    $oNewOpenFileDialog.Multiselect = $Multiselect
    if ($Filter) {
        $oNewOpenFileDialog.Filter = $Filter
    }
    [void]$oNewOpenFileDialog.ShowDialog()
    return $oNewOpenFileDialog.FileNames
}
